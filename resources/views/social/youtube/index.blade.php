...
@section('title', 'YouTube Alerts - SantuyTL')

@section('content')
<div class="page-header">
    <h1 class="page-title"><i class="fa-brands fa-youtube text-red"></i> YouTube Alerts</h1>
    <p class="page-subtitle">Kirim notifikasi otomatis saat ada video baru.</p>
</div>
...
<div class="app-layout">
        ...
        <div class="card-body">
            ...
            @else
                <table class="table" style="width: 100%;">
                    <thead><tr><th>Channel ID</th><th>Discord Channel</th><th>Action</th></tr></thead>
                    <tbody>
                        @foreach($alerts as $alert)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('youtube.destroy', ['guildId' => request()->route('guildId'), 'id' => $alert->id]) }}" method="POST">
                                        @csrf @method('DELETE')
    ...
    <div class="card">
        <div class="card-header"><h3>Add YouTube Alert</h3></div>
        <div class="card-body">
            <form action="{{ route('youtube.store', ['guildId' => request()->route('guildId')]) }}" method="POST">
                @csrf
                <div class="form-group mb-3">
                    <label class="form-label">Channel ID</label>
                    <input type="text" name="identifier" class="form-input" placeholder="UC..." required>
                    <small class="text-muted">YouTube Channel ID (Not handle)</small>
                </div>
                <div class="form-group mb-3">
