...
@section('title', 'TikTok Alerts - SantuyTL')

@section('content')
<div class="page-header">
    <h1 class="page-title"><i class="fa-brands fa-tiktok"></i> TikTok Alerts</h1>
    <p class="page-subtitle">Kirim notifikasi otomatis saat ada video TikTok baru.</p>
</div>
...
<div class="app-layout">
                    ...
                    <tbody>
                        @foreach($alerts as $alert)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('tiktok.destroy', ['guildId' => request()->route('guildId'), 'id' => $alert->id]) }}" method="POST">
                                        @csrf @method('DELETE')
    ...
    <div class="card">
        <div class="card-header"><h3>Add TikTok Alert</h3></div>
        <div class="card-body">
            <form action="{{ route('tiktok.store', ['guildId' => request()->route('guildId')]) }}" method="POST">
                @csrf
                <div class="form-group mb-3">
                    <label class="form-label">Username</label>
                    <input type="text" name="identifier" class="form-input" placeholder="@username" required>
                </div>
                <div class="form-group mb-3">
