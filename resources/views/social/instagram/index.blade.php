...
@section('title', 'Instagram Alerts - SantuyTL')

@section('content')
<div class="page-header">
    <h1 class="page-title"><i class="fa-brands fa-instagram text-pink"></i> Instagram Alerts</h1>
    <p class="page-subtitle">Kirim notifikasi saat ada post baru.</p>
</div>
...
<div class="app-layout">
        ...
        <div class="card-body">
            ...
            @else
                <table class="table" style="width: 100%;">
                    <thead><tr><th>Username</th><th>Discord Channel</th><th>Action</th></tr></thead>
                    <tbody>
                        @foreach($alerts as $alert)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('instagram.destroy', ['guildId' => request()->route('guildId'), 'id' => $alert->id]) }}" method="POST">
                                        @csrf @method('DELETE')
    ...
    <div class="card">
        <div class="card-header"><h3>Add Instagram Alert</h3></div>
        <div class="card-body">
            <form action="{{ route('instagram.store', ['guildId' => request()->route('guildId')]) }}" method="POST">
                @csrf
                <div class="form-group mb-3">
                    <label class="form-label">Username</label>
                    <input type="text" name="identifier" class="form-input" placeholder="instagram" required>
                </div>
                <div class="form-group mb-3">
